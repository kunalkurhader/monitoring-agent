<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Sign in · {{ config('app.name', 'Monitoring Agent') }}</title>@vite(['resources/css/app.css', 'resources/js/app.js'])</head>
...
<main class="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
 ...
 <div class="mb-7">@if(config('app.branding_logo'))<img src="{{ config('app.branding_logo') }}" alt="{{ config('app.name', 'Monitoring Agent') }}" class="mb-5 max-h-16 max-w-52 object-contain">@endif<p class="text-sm font-medium text-emerald-400">{{ config('app.name', 'Monitoring Agent') }}</p><h1 class="mt-2 text-3xl font-semibold">Welcome back</h1><p class="mt-2 text-sm text-slate-400">Sign in with the administrator account created during setup.</p></div>
